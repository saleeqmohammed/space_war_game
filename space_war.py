...
def _show_start():
    ...
    #playbutton_img.set_colorkey(WHITE)
    play_rect = playbutton_img.get_rect()
    play_rect.x = WIDTH/2 - 10
    play_rect.y = HEIGHT/2 + 65
    screen.blit(playbutton_img,play_rect)
    draw_text(screen,"Arrow keys to move, space to fire",14, WIDTH/2,HEIGHT/2)
    draw_text(screen,"press any key to begin",18,WIDTH/2,HEIGHT*0.75)
    draw_text(screen,"Game by Mohammed saleeq",18,WIDTH/2,HEIGHT*0.9)
    draw_text(screen,"mohammedsaleeq.tech",10,WIDTH/2,HEIGHT*0.95)
    pygame.display.flip()
    waiting = True
    while waiting:
        clock.tick(FPS)
...
def show_go_screen(last_score):
    ...
    draw_text(screen,"press Enter to play again",18,WIDTH/2,HEIGHT*0.75)
    pygame.display.flip()
    waiting = True
    while waiting:
        clock.tick(FPS)
        for event in pygame.event.get():
            ...
            if pygame.key.get_pressed()[pygame.K_RETURN]:
                waiting = False
def show_pause():
    draw_text(screen,"PAUSED",62, WIDTH/2,HEIGHT/2)
    draw_text(screen,"PRESS SPACE TO RESUME",14,WIDTH/2,HEIGHT*0.7)
    pygame.display.flip()
    waiting = True
    while waiting:
        clock.tick(FPS)
...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.swap = 1
        self.swap_time = pygame.time.get_ticks()
    def update(self):
        #time out for powerups
        if self.power >=2 and pygame.time.get_ticks() - self.power_time > POWERUP_TIME:
            self.power -=1
            self.power_time = pygame.time.get_ticks()
        if self.swap >=2 and pygame.time.get_ticks() - self.swap_time > POWERUP_TIME:
            self.swap -=1
            self.swap_time = pygame.time.get_ticks()
...
class Pow(pygame.sprite.Sprite):
    def __init__(self,center):
        pygame.sprite.Sprite.__init__(self)
        self.type = random.choice(['shield','gun','swap'])
...
playbutton_img = pygame.image.load(path.join(img_dir,'sprite.png'))
meteor_images = []
meteor_list = [
    'meteor.png',
    'm1.png',
...
powerup_images['swap']= pygame.image.load(path.join(img_dir,'swap.png'))
#Load all the game sound
shoot_snd = pygame.mixer.Sound(path.join(snd_dir,'Laser_Shoot.wav'))
power_snd = pygame.mixer.Sound(path.join(snd_dir,'gun.wav'))
sheild_snd = pygame.mixer.Sound(path.join(snd_dir,'shield.wav'))
swap_snd = pygame.mixer.Sound(path.join(snd_dir,'swap.wav'))
hurt_snd = pygame.mixer.Sound(path.join(snd_dir,'Hit_Hurt3.wav'))
player_explosion = pygame.mixer.Sound(path.join(snd_dir,'rumble1.wav'))
explosion_sounds =[]
